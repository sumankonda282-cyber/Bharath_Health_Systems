...
import { Eye, EyeOff, AlertCircle, Activity, ClipboardList, BedDouble, KeyRound, CheckCircle } from 'lucide-react'
      ...
      <div className="min-h-screen flex" style={{ fontFamily: 'Inter, sans-serif' }}>
        <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-10 text-white"
          ...
          <div>
            ...
            <div className="text-xs font-semibold mt-1 tracking-wider uppercase" style={{ color: '#6ee7b7' }}>CareChart</div>
          </div>
          <div>
            <h2 className="text-4xl font-extrabold leading-tight mb-4">Patient Care,<br /><span style={{ color: '#6ee7b7' }}>Every Shift.</span></h2>
          </div>
        ...
        <div className="flex-1 flex items-center justify-center p-6 bg-gray-50">
          <div className="w-full max-w-md">
            <div className="lg:hidden text-center mb-8">
              ...
              <div className="text-xs font-semibold tracking-wider uppercase mt-1" style={{ color: '#065F46' }}>CareChart</div>
            </div>
            <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-8">
              {forgotSent ? (
                <div className="text-center space-y-4">
                  ...
                  <p className="text-sm text-gray-600">Your request has been sent to your clinic manager. They will provide you with a temporary password.</p>
                  <button onClick={() => { setMode('login'); setForgotSent(false); setForgotForm({ identifier: '', note: '' }) }}
                    className="w-full py-3 rounded-xl font-semibold text-sm text-white" style={{ background: '#065F46' }}>
                    Back to Login
                  </button>
                </div>
              ) : (
                <>
                  ...
                  <p className="text-gray-500 text-sm mb-6">Your request will be sent to your clinic manager.</p>
                  <form onSubmit={submitForgot} className="space-y-4">
                    ...
                    <button type="submit" disabled={forgotLoading}
                      className="w-full py-3 rounded-xl font-semibold text-sm text-white disabled:opacity-50" style={{ background: '#065F46' }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-10 text-white"
        ...
        <div>
          <h2 className="text-4xl font-extrabold leading-tight mb-4">
            Patient Care,<br /><span style={{ color: '#6ee7b7' }}>Every Shift.</span>
          </h2>
          <p className="text-emerald-200 text-lg mb-8">Chart vitals, write nursing notes, manage medications, and hand off seamlessly — all in one place.</p>
          {FEATURES.map(({ icon: Icon, text }) => (
            <div key={text} className="flex items-center gap-3 mb-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(110,231,183,0.2)' }}>
          ...
          <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-8">
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                ...
                <input className="input" placeholder="username, mobile or email"
                  value={form.identifier} onChange={e => setForm(f => ({ ...f, identifier: e.target.value }))} required autoFocus />
              </div>
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="label mb-0">Password</label>
                  <button type="button" onClick={() => setMode('forgot')}
                    className="text-xs flex items-center gap-1 hover:underline" style={{ color: '#CC1414' }}>
              ...
              <button type="submit" disabled={loading}
                ...
                {loading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Signing in…</> : 'Sign In to Ward'}
              </button>
            </form>
          </div>
        </div>

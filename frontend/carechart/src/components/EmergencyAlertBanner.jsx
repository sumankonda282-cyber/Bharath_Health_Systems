...
const TRIAGE_BG    = { red: '#dc2626', orange: '#f97316', yellow: '#ca8a04', green: '#16a34a' }
const TRIAGE_LABEL = { red: 'CRITICAL', orange: 'URGENT', yellow: 'SEMI-URGENT', green: 'STABLE' }
...
export default function EmergencyAlertBanner() {
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(id); window.removeEventListener('carechart:refresh', h) }
  }, [fetchAlerts])
  ...
  useEffect(() => {
    if (alerts.length > 0) {
      ...
      if (ctx) {
        ...
        doBeep()
        if (!beepTimer.current) beepTimer.current = setInterval(doBeep, 3500)
      }
    } else {
  ...
  useEffect(() => () => { if (beepTimer.current) clearInterval(beepTimer.current) }, [])
    ...
    <div className="flex-shrink-0">
      {alerts.map(a => (
        <div
          ...
          style={{ background: TRIAGE_BG[a.triage_level] || '#dc2626' }}
        >
          ...
          <div className="flex-1 min-w-0 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-sm">
            <span className="font-bold text-xs px-2 py-0.5 rounded uppercase tracking-wide" style={{ background: 'rgba(255,255,255,0.2)' }}>
            ...
            {a.chief_complaint && <span className="text-xs opacity-75 truncate max-w-xs">· {a.chief_complaint}</span>}
            {a.eta_minutes     && <span className="text-xs opacity-75">ETA {a.eta_minutes} min</span>}
          </div>
          <button
            onClick={() => acknowledge(a)}
            disabled={acking[a.id]}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-sm font-bold flex-shrink-0 transition disabled:opacity-60"
            style={{ background: 'rgba(255,255,255,0.2)', borderColor: 'rgba(255,255,255,0.3)' }}
          >
            {acking[a.id] ? <Loader2 size={13} className="animate-spin" /> : <CheckCircle2 size={13} />}
            Accept
          </button>
        </div>
      ))}

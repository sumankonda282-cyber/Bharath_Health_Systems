...
export default function PatientList({ selectedId, onSelect, navigateTo = false }) {
  ...
  return (
    <ul className="divide-y divide-gray-100">
      {patients.map(p => {
        ...
        const bed  = p.bed_number || p.bed_label || p.bed?.label || '—'
        const mrn  = p.uhid || p.patient?.mrn || p.mrn || ''
        return (
          <li key={p.id}>
            <button

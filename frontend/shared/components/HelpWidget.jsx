...
/**
 * Shared Help & Support modal — portal-agnostic, prop-driven.
 * Props: open, onClose, api (axios client), user, portalSource (e.g. 'Laboratory').
 * "Report an Issue" POSTs a maintenance/support request; "Contact IT" shows the
 * support line. No portal-specific imports, so every portal can reuse it.
 */
...
export default function HelpWidget({ open, onClose, api, user, portalSource = 'Portal' }) {
  ...
  const submit = async e => {
    e.preventDefault()
    if (!form.title.trim() || !api) return
    setSaving(true); setError('')
    try {
      await api.post('/support/maintenance-request', { ...form, portal_source: portalSource })
      setDone(true)
    } catch (err) {
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ background: NAVY }}>
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="text-white/50 text-xs ml-1">· {portalSource}</span>
          </div>
        ...
        <div className="p-5">
          {!path && (
            <div className="space-y-3">
              ...
              <button onClick={() => setPath('internal')}
                ...
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: '#dbe7fb' }}>
                  <Wrench size={18} style={{ color: NAVY }} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-gray-800 text-sm">Report an Issue</div>
                  <div className="text-xs text-gray-500 mt-0.5">Equipment, facility, or hospital IT problem</div>
                </div>
          ...
          {path === 'internal' && !done && (
            <form onSubmit={submit} className="space-y-3">
              <div>
                ...
                <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                ...
                <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="e.g. Lab Room 2, Reception" value={form.location}
                  onChange={e => setForm(f => ({ ...f, location: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1">Details (optional)</label>
                <textarea className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none"
                  rows={3} placeholder="Describe the problem in more detail..." value={form.description}
                  onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
              </div>
              <div className="text-xs text-gray-400">
                Submitting as <span className="font-medium text-gray-600">{user?.full_name || user?.email}</span> · {portalSource}
              </div>
              {error && (
          ...
          {done && (
            <div className="text-center py-6">
              ...
              <div className="text-sm font-semibold text-gray-800 mb-1">Request submitted</div>
              <div className="text-xs text-gray-500 mb-4">Our team will follow up shortly.</div>
              <button onClick={close} className="px-5 py-2 rounded-xl text-white text-sm font-semibold" style={{ background: NAVY }}>Done</button>
            </div>
          )}
        </div>
      </div>

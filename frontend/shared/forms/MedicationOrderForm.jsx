// Universal medication-order form — shared by provider IPD (AdmissionChart),
// CareChart, and provider OPD (Patient Chart). Portal-specific dependencies are
// injected as props: `api` (HTTP client), `currentUser` (for recent-drug memory),
// and an optional `requestPin` gate (inpatient nursing only — OPD passes none).
import { useState, useEffect, useCallback, useRef } from 'react'
import { checkInteractions, checkAllergyConflict } from './drugInteractions'
import {
...
export default function MedicationOrderForm({
  ...
  // drug search — debounced + OpenFDA hybrid fallback
  useEffect(() => {
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      try {
        ...
        // OpenFDA fallback when India seed has < 3 results
  ...
  // CDS check — fires when drug + dose + route all set
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    if (hasMajor && !overrideReason) {
      setError('Enter a clinical override reason for the major/contraindicated interaction')
      return
    }

    // PIN gate — inpatient nursing only. OPD passes no requestPin, so it's skipped.
    if (requestPin) {
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  className="w-full border rounded-xl pl-9 pr-10 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-600"
            ...
            {/* CDS alerts (dose limit, duplication, contraindications) */}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input
                  className="w-full border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600"
                  style={{ borderColor: '#e5e7eb' }}
                  placeholder="e.g. 500"
                  value={form.dose}
                ...
                {weightDose && (
                  <p className="text-[10px] mt-1" style={{ color: GREEN }}>
                    = {weightDose} mg total ({patientData.weight_kg} kg)
                  </p>
                )}
              </div>
              <div>
              ...
              <div className="pb-1">
                {totalDoses !== null ? (
                  <div className="text-center py-2 rounded-xl border font-semibold text-sm"
                    style={{ background: '#f0fdf4', borderColor: '#bbf7d0', color: GREEN }}>
            ...
            <div className="px-4 py-4 space-y-4">
              ...
              {searching && !searchRes.length && (
                <div className="flex flex-col items-center py-12 text-center">
                  <Loader2 size={24} className="text-green-400 mb-3 animate-spin" />
          ...
          <div className="flex gap-3 flex-shrink-0">
            ...
            <button
              ...
              {saving
                ...
                : <>{requestPin && <Lock size={13} />}{initialValues ? 'Update Order' : 'Add Order'}</>
              }
            </button>
          </div>
        </div>

...
// Always-on safety banner: surfaces unacknowledged panic lab values the current
// doctor ordered, on every page. Hidden entirely when there is nothing to show.
export default function CriticalResultsBanner() {
  ...
  const load = useCallback(() => {
    api.get('/lab/critical-alerts', { params: { mine: true } })
    ...
    <div className="border-b border-red-200 bg-red-50">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center gap-2 px-4 py-2 text-left">
        ...
        <span className="text-sm font-semibold text-red-700">
          {alerts.length} critical lab result{alerts.length > 1 ? 's' : ''} need review
        </span>

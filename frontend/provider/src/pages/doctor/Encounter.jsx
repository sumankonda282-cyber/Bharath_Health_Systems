...
export default function Encounter() {
  ...
  const completeEncounter = async () => {
    ...
    try {
      const payload = {
        soap: { ...soap, appointment_id: parseInt(id), diagnoses: diagnoses.map(d => ({ label: d.label, icd: d.icd })) },
    ...
    <div className="max-w-4xl">
      ...
      {tab === 'soap' && (
        <div className="card p-6 space-y-4">
          ...
          {/* A — Assessment / Diagnosis: structured typeahead + freetext notes */}
          <div>
            <label className="label">A — Assessment / Diagnosis</label>
            <DiagnosisInput
              value={soap.assessment}
              onChange={(notes, selected) => {
                setSoap(s => ({ ...s, assessment: notes }))

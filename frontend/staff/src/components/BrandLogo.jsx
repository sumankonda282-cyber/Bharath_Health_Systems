    ...
    <div className="inline-flex items-center" style={{ gap: 4 }}>
      ...
      {showText && (
        <div className="flex flex-col leading-none" style={{ gap: 1 }}>
          <span className={`font-extrabold ${s.name}`} style={{ color: nameColor, letterSpacing: '-0.02em' }}>
            <span style={{ color: '#CC1414' }}>BHarath</span>{' '}Health
          </span>
          <span className={`italic ${s.sub} text-right`} style={{ color: subColor, letterSpacing: '0.04em' }}>
            Systems
          </span>
        </div>
      )}

...
export default function PlanBuilder({ onClose, addToast }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/70 flex items-center justify-center p-4">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl h-[88vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-800 flex-shrink-0">
          <div className="flex items-center gap-2">
            <Package size={18} className="text-blue-400" />
            <h2 className="text-sm font-bold text-white">Plan Builder</h2>
            <span className="text-xs text-gray-500">— compose à-la-carte plans</span>
          </div>
        ...
        <div className="flex-1 flex min-h-0">
          {/* List */}
          <div className="w-60 border-r border-gray-800 flex flex-col flex-shrink-0">
            <button onClick={openNew} className="m-3 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold">
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-2 space-y-1">
              {loading ? (
                ...
                <p className="text-xs text-gray-500 text-center py-8">No plans yet</p>
              ) : plans.map(p => (
                <button key={p.id} onClick={() => openEdit(p)}
                  className={`w-full text-left px-3 py-2 rounded-lg transition-colors ${editing?.id === p.id ? 'bg-gray-800' : 'hover:bg-gray-800/50'}`}>
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: p.color || '#64748b' }} />
                    <span className="text-sm text-white font-medium truncate">{p.name}</span>
                    {!p.is_active && <span className="text-[10px] text-gray-500">off</span>}
                  </div>
                  <div className="text-[11px] text-gray-500 ml-4">{p.key}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-5">
            {!editing ? (
              ...
              <div className="space-y-4 max-w-2xl">
                    ...
                    {moduleKeys.map(k => {
                      ...
                      return (
                        <button key={k} type="button" onClick={() => toggleModule(k)}
                          className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition-colors ${on ? 'bg-emerald-900/30 border-emerald-700 text-emerald-200' : 'bg-gray-800 border-gray-700 text-gray-400'}`}>
                          <span className={`w-4 h-4 rounded flex items-center justify-center ${on ? 'bg-emerald-500' : 'bg-gray-700'}`}>{on && <Check size={11} className="text-white" />}</span>
                ...
                {/* Limits */}
                <div>
                  <label className={lbl}>Limits <span className="text-gray-600">(0 or 999 = unlimited)</span></label>
                  <div className="grid grid-cols-3 gap-3">
                    {LIMIT_FIELDS.map(f => (
                      <div key={f.key}>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><span className="text-[11px] text-gray-500">Monthly (flat)</span><input type="number" className={inp} value={form.monthly_price} onChange={e => set('monthly_price', e.target.value)} /></div>
                    <div><span className="text-[11px] text-gray-500">Annual (flat)</span><input type="number" className={inp} value={form.annual_price} onChange={e => set('annual_price', e.target.value)} /></div>
                    <div><span className="text-[11px] text-gray-500">Monthly / seat</span><input type="number" className={inp} value={form.monthly_price_per_seat} onChange={e => set('monthly_price_per_seat', e.target.value)} /></div>
                    <div><span className="text-[11px] text-gray-500">Annual / seat</span><input type="number" className={inp} value={form.annual_price_per_seat} onChange={e => set('annual_price_per_seat', e.target.value)} /></div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 items-end">
                  <div>
                    ...
                    <input type="color" className="w-full h-9 bg-gray-800 border border-gray-700 rounded-lg" value={form.color || '#3B82F6'} onChange={e => set('color', e.target.value)} />
                  </div>
                  <div>
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => set('is_public', !form.is_public)}
                      className={`flex-1 flex items-center justify-center gap-1.5 px-2 py-2 rounded-lg text-xs font-medium border ${form.is_public ? 'bg-blue-900/30 border-blue-700 text-blue-300' : 'bg-gray-800 border-gray-700 text-gray-400'}`}>
                      {form.is_public ? <Eye size={13} /> : <EyeOff size={13} />} {form.is_public ? 'Public' : 'Private'}
                    </button>
                    <button type="button" onClick={() => set('is_active', !form.is_active)}
                      className={`flex-1 px-2 py-2 rounded-lg text-xs font-medium border ${form.is_active ? 'bg-emerald-900/30 border-emerald-700 text-emerald-300' : 'bg-gray-800 border-gray-700 text-gray-400'}`}>
                ...
                <div className="flex gap-3 pt-1">
                  <button onClick={() => setEditing(null)} className="px-4 py-2 rounded-lg border border-gray-700 text-sm text-gray-300 hover:bg-gray-800">Cancel</button>
                  <button onClick={save} disabled={saving}
                    className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold disabled:opacity-50">
                    {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} {editing.__new ? 'Create plan' : 'Save changes'}
                  </button>
                </div>
                <p className="text-[11px] text-gray-600">Editing a plan never re-prices existing subscribers — they keep their agreed terms until renewal (grandfathered).</p>
              </div>
            )}
          </div>
        </div>

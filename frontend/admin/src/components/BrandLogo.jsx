    ...
    <div className="inline-flex items-center" style={{ gap: 4 }}>
      ...
      {showText && (
        <div className="flex flex-col leading-none" style={{ gap: 1 }}>
          <span className={`font-extrabold ${s.name}`} style={{ color: '#ffffff', letterSpacing: '-0.02em' }}>
            <span style={{ color: '#CC1414' }}>BHarath</span>{' '}Health
          </span>
          <span className={`italic ${s.sub} text-right`} style={{ color: 'rgba(255,255,255,0.6)', letterSpacing: '0.04em' }}>
            Systems
          </span>
        </div>
      )}

...
export function ConfirmTransfersModal({ onClose, addToast }) {
  ...
  return (
    <Shell title="Confirm Bank Transfers" icon={Landmark} onClose={onClose}>
      {loading ? (
        <div className="flex justify-center py-10"><Loader2 size={20} className="animate-spin text-faint" /></div>
      ) : rows.length === 0 ? (
        <p className="text-sm text-faint text-center py-10">No bank transfers awaiting confirmation.</p>
      ) : (
        <div className="space-y-2">
          {rows.map(inv => (
            <div key={inv.id} className="flex items-center justify-between surface-2 border border-app rounded-lg px-3 py-2.5">
              <div className="min-w-0">
                <p className="text-sm text-app font-medium truncate">{inv.clinic_name || `Health Center #${inv.clinic_id}`}</p>
                <p className="text-[11px] text-dim truncate">{inv.plan_key} · {inv.billing_cycle} · ref {inv.reference || '—'}</p>
              </div>
              <div className="flex items-center gap-3 flex-shrink-0">
                <span className="text-sm font-bold text-app">{inr(inv.amount)}</span>
                <button onClick={() => confirm(inv)} disabled={busy === inv.id}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-app text-xs font-semibold disabled:opacity-50">
    ...
    <Shell title="Grant Free Access (Comp)" icon={Gift} onClose={onClose} maxW="max-w-md">
      <div className="space-y-3">
        ...
        <div className="flex gap-3 pt-1">
          <button onClick={onClose} className="px-4 py-2 rounded-lg border border-app text-sm text-dim hover:surface-2">Cancel</button>
          <button onClick={submit} disabled={saving}
            className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-app text-sm font-semibold disabled:opacity-50">
            {saving ? <Loader2 size={14} className="animate-spin" /> : <Gift size={14} />} Grant free access
          </button>
        </div>
        <p className="text-[11px] text-faint">Fully audited and reversible — assign a paid plan or let it expire to end the comp.</p>
      </div>
    </Shell>
  )
}
...
function Shell({ title, icon: Icon, onClose, children, maxW = 'max-w-lg' }) {
  return (
    <div className="fixed inset-0 z-[80] bg-black/70 flex items-center justify-center p-4">
      <div className={`surface border border-app rounded-2xl shadow-2xl w-full ${maxW} max-h-[85vh] flex flex-col`}>
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-app flex-shrink-0">
          <div className="flex items-center gap-2"><Icon size={17} className="text-blue-400" /><h2 className="text-sm font-bold text-app">{title}</h2></div>

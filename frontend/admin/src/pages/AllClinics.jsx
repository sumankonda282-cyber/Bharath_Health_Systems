...
function ClinicDetailDrawer({ clinic, onClose }) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-[70vw] max-w-3xl bg-gray-900 border-l border-gray-800 z-50 overflow-y-auto shadow-2xl flex flex-col">
        {/* Header */}
        <div className="sticky top-0 bg-gray-900 border-b border-gray-800 px-6 py-4 flex items-start justify-between gap-4 z-10">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-lg font-bold text-white truncate">{clinic.name}</h2>
            ...
            <p className="text-xs text-gray-500">{[clinic.city, clinic.state].filter(Boolean).join(', ') || '—'}</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-gray-400 hover:text-white hover:bg-gray-800 transition-colors flex-shrink-0">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { label: 'Doctors',  value: <span className="text-white font-bold text-lg">{clinic.doctor_count ?? '—'}</span> },
  ...
  return (
    <div className="space-y-2.5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <h1 className="text-base font-bold text-white">All Health Centers</h1>
        <div className="flex gap-0.5 bg-gray-900 border border-gray-800 p-0.5 rounded-lg overflow-x-auto">
          {STATUS_TABS.map(s => (
            <button
              key={s}
              onClick={() => setTab(s)}
            ...
            >
              ...
              {!loading && counts[s] !== undefined && (
                <span className={`text-[10px] ${tab === s ? 'opacity-70' : 'text-gray-600'}`}>
                  {counts[s]}
                </span>
              )}
            </button>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {hasFilters && (
          <button onClick={clearFilters} className="text-xs text-gray-400 hover:text-white underline">Clear</button>
        )}
              ...
              <tbody className="divide-y divide-gray-800/50">
                {filtered.map(c => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3 text-gray-400 text-xs">{c.specialty || '—'}</td>
                    <td className="px-3 py-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold capitalize ${STATUS_BADGE[c.status] || 'bg-gray-700 text-gray-400'}`}>
                        {c.status}
                      </span>
                    </td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3 text-center text-white font-semibold">{c.doctor_count ?? '—'}</td>

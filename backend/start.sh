...
safe_cols = [
    ...
    \"CREATE TABLE IF NOT EXISTS assessment_forms (id SERIAL PRIMARY KEY, title VARCHAR(300) NOT NULL, description TEXT, category VARCHAR(50) DEFAULT \'general\', subcategory VARCHAR(100), icon VARCHAR(50), schema JSONB NOT NULL DEFAULT \'{}\'::jsonb, scoring_config JSONB, iview_config JSONB, alert_rules JSONB, translations JSONB, status VARCHAR(20) DEFAULT \'draft\', version INTEGER DEFAULT 1, is_template BOOLEAN DEFAULT FALSE, is_iview_enabled BOOLEAN DEFAULT FALSE, requires_cosign BOOLEAN DEFAULT FALSE, time_limit_minutes INTEGER, created_by INTEGER REFERENCES staff(id), created_by_admin INTEGER REFERENCES platform_admins(id), clinic_id INTEGER REFERENCES clinics(id), parent_form_id INTEGER REFERENCES assessment_forms(id), published_at TIMESTAMP, retired_at TIMESTAMP, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS assessment_form_versions (id SERIAL PRIMARY KEY, form_id INTEGER REFERENCES assessment_forms(id) NOT NULL, version INTEGER NOT NULL, schema JSONB NOT NULL, scoring_config JSONB, published_by INTEGER REFERENCES platform_admins(id), published_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS form_pool (id SERIAL PRIMARY KEY, form_id INTEGER REFERENCES assessment_forms(id) NOT NULL, clinic_id INTEGER REFERENCES clinics(id), assigned_by INTEGER REFERENCES platform_admins(id), assigned_at TIMESTAMP DEFAULT NOW(), is_active BOOLEAN DEFAULT TRUE, UNIQUE(form_id, clinic_id))\",
    \"CREATE TABLE IF NOT EXISTS form_assignments (id SERIAL PRIMARY KEY, form_id INTEGER REFERENCES assessment_forms(id) NOT NULL, form_version INTEGER NOT NULL, clinic_id INTEGER REFERENCES clinics(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, appointment_id INTEGER REFERENCES appointments(id), admission_id INTEGER REFERENCES admissions(id), assigned_by INTEGER REFERENCES staff(id) NOT NULL, assigned_to_role VARCHAR(30), due_at TIMESTAMP, status VARCHAR(20) DEFAULT \'pending\', priority VARCHAR(10) DEFAULT \'routine\', notes TEXT, assigned_at TIMESTAMP DEFAULT NOW(), completed_at TIMESTAMP)\",
    \"CREATE TABLE IF NOT EXISTS form_submissions (id SERIAL PRIMARY KEY, form_id INTEGER REFERENCES assessment_forms(id) NOT NULL, form_version INTEGER NOT NULL, assignment_id INTEGER REFERENCES form_assignments(id), clinic_id INTEGER REFERENCES clinics(id) NOT NULL, patient_id INTEGER REFERENCES patients(id) NOT NULL, appointment_id INTEGER REFERENCES appointments(id), admission_id INTEGER REFERENCES admissions(id), submitted_by INTEGER REFERENCES staff(id) NOT NULL, cosigned_by INTEGER REFERENCES staff(id), cosigned_at TIMESTAMP, data JSONB NOT NULL DEFAULT \'{}\'::jsonb, scores JSONB, alerts_fired JSONB, is_draft BOOLEAN DEFAULT FALSE, submitted_at TIMESTAMP DEFAULT NOW(), charted_at TIMESTAMP, source VARCHAR(20) DEFAULT \'provider\', created_at TIMESTAMP DEFAULT NOW())\",

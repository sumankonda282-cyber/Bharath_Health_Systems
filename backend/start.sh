...
critical = [
    ...
    'ALTER TABLE clinics ADD COLUMN IF NOT EXISTS entitlement_overrides JSONB',
    'ALTER TABLE clinics ADD COLUMN IF NOT EXISTS billing_email VARCHAR(150)',
...
safe_cols = [
    ...
    \"ALTER TABLE admissions ADD COLUMN IF NOT EXISTS discharge_checklist JSON\",
    \"ALTER TABLE admissions ADD COLUMN IF NOT EXISTS discharge_notes TEXT\",
    \"ALTER TABLE admissions ADD COLUMN IF NOT EXISTS discharge_destination VARCHAR(120)\",
    ...
    \"ALTER TABLE clinics ADD COLUMN IF NOT EXISTS bridge_last_seen TIMESTAMP\",
    ...
    \"ALTER TABLE patient_referrals ADD COLUMN IF NOT EXISTS to_clinic_name VARCHAR(255)\",
    \"ALTER TABLE patient_referrals ADD COLUMN IF NOT EXISTS to_hc_id VARCHAR(20)\",
    \"ALTER TABLE patient_referrals ADD COLUMN IF NOT EXISTS to_specialty VARCHAR(150)\",
    \"ALTER TABLE patient_referrals ADD COLUMN IF NOT EXISTS to_doctor_name VARCHAR(200)\",
    \"ALTER TABLE patient_referrals ADD COLUMN IF NOT EXISTS current_medications TEXT\",
    \"ALTER TABLE patient_referrals ADD COLUMN IF NOT EXISTS relevant_investigations TEXT\",
    \"ALTER TABLE patient_referrals ADD COLUMN IF NOT EXISTS patient_bh_id VARCHAR(50)\",
    \"ALTER TABLE appointments ADD COLUMN IF NOT EXISTS triage_level VARCHAR(10)\",
    \"ALTER TABLE vital_signs ADD COLUMN IF NOT EXISTS blood_sugar NUMERIC(6,2)\",
    \"CREATE TABLE IF NOT EXISTS clinical_assessments (id SERIAL PRIMARY KEY, clinic_id INTEGER NOT NULL REFERENCES clinics(id), branch_id INTEGER REFERENCES branches(id), patient_id INTEGER REFERENCES patients(id), encounter_id VARCHAR(50), assessment_type VARCHAR(100), data JSONB DEFAULT '{}'::jsonb, created_by INTEGER REFERENCES staff(id), created_at TIMESTAMP DEFAULT NOW())\",
    ...
    \"CREATE TABLE IF NOT EXISTS lab_critical_alerts (id SERIAL PRIMARY KEY, clinic_id INTEGER REFERENCES clinics(id), order_id INTEGER REFERENCES lab_orders(id), patient_id INTEGER REFERENCES patients(id), ordered_by INTEGER REFERENCES staff(id), test_name VARCHAR(200), value VARCHAR(100), flag VARCHAR(4), description TEXT, created_at TIMESTAMP DEFAULT NOW(), acknowledged_by INTEGER REFERENCES staff(id), acknowledged_at TIMESTAMP)\",
    ...
    \"ALTER TABLE staff ADD COLUMN IF NOT EXISTS manager_scope VARCHAR(20)\",
...
\"ALTER TABLE drug_interactions ADD COLUMN IF NOT EXISTS interaction_type VARCHAR(30) DEFAULT 'drug-drug'\",
    ...
    \"CREATE TABLE IF NOT EXISTS plans (id SERIAL PRIMARY KEY, key VARCHAR(40) UNIQUE NOT NULL, name VARCHAR(80) NOT NULL, description TEXT, color VARCHAR(20), currency VARCHAR(8) DEFAULT 'INR', monthly_price NUMERIC(12,2) DEFAULT 0, annual_price NUMERIC(12,2) DEFAULT 0, monthly_price_per_seat NUMERIC(12,2) DEFAULT 0, annual_price_per_seat NUMERIC(12,2) DEFAULT 0, modules JSONB NOT NULL DEFAULT '{}', limits JSONB NOT NULL DEFAULT '{}', features JSONB, is_public BOOLEAN DEFAULT TRUE, is_active BOOLEAN DEFAULT TRUE, sort_order INTEGER DEFAULT 0, version INTEGER DEFAULT 1, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS clinic_subscriptions (id SERIAL PRIMARY KEY, clinic_id INTEGER UNIQUE NOT NULL REFERENCES clinics(id), plan_id INTEGER REFERENCES plans(id), plan_key VARCHAR(40), status VARCHAR(20) DEFAULT 'active', billing_cycle VARCHAR(12) DEFAULT 'monthly', seats INTEGER DEFAULT 0, price_snapshot NUMERIC(12,2) DEFAULT 0, entitlements_snapshot JSONB, current_period_start DATE, current_period_end DATE, grace_until DATE, auto_renew BOOLEAN DEFAULT TRUE, cancel_at DATE, is_waived BOOLEAN DEFAULT FALSE, waived_reason TEXT, waived_by INTEGER, waived_until DATE, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS subscription_invoices (id SERIAL PRIMARY KEY, clinic_id INTEGER NOT NULL REFERENCES clinics(id), plan_id INTEGER REFERENCES plans(id), plan_key VARCHAR(40), billing_cycle VARCHAR(12) DEFAULT 'monthly', seats INTEGER DEFAULT 0, amount NUMERIC(12,2) NOT NULL, currency VARCHAR(8) DEFAULT 'INR', status VARCHAR(20) DEFAULT 'pending', method VARCHAR(20), gateway_order_id VARCHAR(120), gateway_payment_id VARCHAR(120), reference VARCHAR(200), period_from DATE, period_to DATE, notes TEXT, created_by INTEGER, paid_at TIMESTAMP, created_at TIMESTAMP DEFAULT NOW())\",
    \"CREATE TABLE IF NOT EXISTS webhook_events (id SERIAL PRIMARY KEY, provider VARCHAR(20) DEFAULT 'razorpay', event_id VARCHAR(120) UNIQUE, event_type VARCHAR(60), payload JSONB, processed BOOLEAN DEFAULT FALSE, created_at TIMESTAMP DEFAULT NOW())\",
    ...
    \"CREATE TABLE IF NOT EXISTS staff_form_favorites (id SERIAL PRIMARY KEY, clinic_id INTEGER NOT NULL REFERENCES clinics(id), staff_id INTEGER NOT NULL REFERENCES staff(id), form_id INTEGER NOT NULL REFERENCES assessment_forms(id) ON DELETE CASCADE, ward_id INTEGER REFERENCES wards(id), scope VARCHAR(20) NOT NULL DEFAULT 'personal', created_at TIMESTAMP DEFAULT NOW())\",
    \"ALTER TABLE staff_form_favorites ADD COLUMN IF NOT EXISTS ward_id INTEGER REFERENCES wards(id)\",
    ...
    \"CREATE UNIQUE INDEX IF NOT EXISTS uq_fav_ward ON staff_form_favorites(ward_id, form_id) WHERE scope = 'ward'\",
    ...
    \"CREATE TABLE IF NOT EXISTS clinical_field_definitions (id SERIAL PRIMARY KEY, field_id VARCHAR(80) NOT NULL, clinic_id INTEGER REFERENCES clinics(id), \\\"group\\\" VARCHAR(50), data_type VARCHAR(20), unit VARCHAR(30), code VARCHAR(40), code_system VARCHAR(20), labels JSON, aliases JSON, components JSON, is_active BOOLEAN DEFAULT TRUE, created_at TIMESTAMP DEFAULT NOW(), updated_at TIMESTAMP DEFAULT NOW())\",
    \"ALTER TABLE clinical_field_definitions ADD COLUMN IF NOT EXISTS code VARCHAR(40)\",
    \"ALTER TABLE clinical_field_definitions ADD COLUMN IF NOT EXISTS code_system VARCHAR(20)\",
    \"ALTER TABLE clinical_field_definitions ADD COLUMN IF NOT EXISTS components JSON\",
...
for _sql in [
    ...
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS clinic_id INTEGER REFERENCES clinics(id)\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS form_version INTEGER DEFAULT 1\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS data JSONB DEFAULT '{}'\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS scores JSONB\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS alerts_fired JSONB DEFAULT '[]'\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS is_draft BOOLEAN DEFAULT FALSE\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS source VARCHAR(30) DEFAULT 'provider'\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS charted_at TIMESTAMP\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS admission_id INTEGER\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS signed_at TIMESTAMP\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS signed_by INTEGER\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS record_status VARCHAR(20) DEFAULT 'active'\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS amends_id INTEGER\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS root_id INTEGER\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS amend_reason TEXT\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS amended_by INTEGER\",
    \"ALTER TABLE form_submissions ADD COLUMN IF NOT EXISTS amended_at TIMESTAMP\",
    \"CREATE INDEX IF NOT EXISTS idx_form_submissions_draft ON form_submissions(form_id, submitted_by, is_draft)\",
    \"CREATE INDEX IF NOT EXISTS idx_form_submissions_status ON form_submissions(record_status)\",
    \"CREATE INDEX IF NOT EXISTS idx_form_submissions_root ON form_submissions(root_id)\",

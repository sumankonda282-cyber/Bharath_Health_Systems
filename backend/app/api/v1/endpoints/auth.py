...
from app.models.models import Staff, PatientUser, PlatformAdmin, Clinic, BHProfile, BHStateGroup, BHIDSequence, AuditLog, PasswordResetRequest
...
def change_staff_password(
    ...
    current.hashed_password = hash_password(payload.new_password)
    current.token_version = (current.token_version or 1) + 1
    current.is_first_login = False
    db.commit()
...
def staff_logout_all(db: Session = Depends(get_db), current=Depends(get_current_staff)):
    current.token_version = (current.token_version or 1) + 1
    db.commit()
...
def pin_setup(body: dict, db: Session = Depends(get_db), current=Depends(get_current_staff)):
    pin = str(body.get("pin", ""))

    if len(pin) != 4 or not pin.isdigit():
        raise HTTPException(status_code=400, detail="PIN must be exactly 4 digits")

    # Block sequential digits (1234, 4321) and repeated (1111, 2222)
    digits = [int(c) for c in pin]
    if digits == sorted(digits) and all(digits[i+1] - digits[i] == 1 for i in range(3)):
        raise HTTPException(status_code=400, detail="PIN cannot be sequential digits (e.g. 1234)")
    if digits == sorted(digits, reverse=True) and all(digits[i] - digits[i+1] == 1 for i in range(3)):
        raise HTTPException(status_code=400, detail="PIN cannot be sequential digits (e.g. 4321)")
    if len(set(digits)) == 1:
        raise HTTPException(status_code=400, detail="PIN cannot be all same digits (e.g. 1111)")

    current.pin_hash = hash_password(pin)
    current.pin_set_at = datetime.utcnow()
    current.pin_reset_required = False
    db.commit()
    return {"detail": "PIN set successfully"}


@router.post("/staff/forgot-password")
def staff_forgot_password(body: dict, db: Session = Depends(get_db)):
    """Staff submits a password reset request. Manager sees it in their queue."""
    identifier = body.get("identifier", "").strip()
    note = body.get("note", "")

    # Always return success to prevent user enumeration
    if not identifier:
        return {"message": "If an account was found, a reset request has been sent to your clinic manager."}

    staff = db.query(Staff).filter(
        (Staff.mobile == identifier) | (Staff.email == identifier) | (Staff.username == identifier)
    ).first()

    if staff and staff.is_active:
        # Check if there's already a pending request
        existing = db.query(PasswordResetRequest).filter(
            PasswordResetRequest.staff_id == staff.id,
...
def staff_pin_status(current=Depends(get_current_staff)):
    return {
        "has_pin": bool(getattr(current, 'pin_hash', None)),
        "pin_reset_required": getattr(current, 'pin_reset_required', False),
        "is_locked": bool(getattr(current, 'pin_locked_until', None) and current.pin_locked_until > datetime.utcnow()),
        "locked_until": getattr(current, 'pin_locked_until', None),
        "pin_set_at": getattr(current, 'pin_set_at', None),
    }

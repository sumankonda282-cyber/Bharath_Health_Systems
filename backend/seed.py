...
Uses INSERT ... ON CONFLICT DO NOTHING so re-runs never duplicate or error.
Run: python seed.py
"""
import sys, os
...
def seed():
    ...
    try:
        ...
        if not _exists(db, PlatformAdmin, email="superadmin@bharathealth.com"):
            ...
            # Always refresh password hash in case bcrypt library changed
        ...
        # ── Staff — always refresh passwords so hashes match current library ──
        staff_defs = [
        ...
        print("  Provider (Doctor): drpriya@apollodemo.com / Doctor@123")
